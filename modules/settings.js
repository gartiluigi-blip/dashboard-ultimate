...
function backup(root){let data='';try{data=Store.exportJson()}catch(e){data=JSON.stringify({error:'export_failed',message:String(e)},null,2)}const c=card('Backup complet','Copie tout le stockage du dashboard avant modification.');const t=el('textarea',{class:'input',rows:'14'},data);c.append(t);c.append(el('div',{class:'row'},[el('button',{class:'btn green',onclick:()=>navigator.clipboard.writeText(data).then(()=>toast('Backup copié'))},'Copier backup'),el('button',{class:'btn',onclick:()=>download(data)},'Télécharger JSON')]));root.append(c)}
