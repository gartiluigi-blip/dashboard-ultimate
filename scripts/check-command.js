...
const files = ['v6/app.js', 'v6/content.js', 'v6/rules.js', 'v6/store.js'];
...
for (const token of ['renderToday', 'renderAthlete', 'renderTrading', 'renderStudy', 'renderLibrary', 'renderNutrition', 'renderMoney', 'renderSystem']) {
...
for (const token of ['todayOrders', 'tradingReadiness', 'athleteCoverage', 'propFirmStats']) {
  if (!rules.includes(token)) throw new Error(`Rules contract missing: ${token}`);
}
for (const token of ['SCHEMA_VERSION = 2', 'trading:', 'reading:', 'purgeDeletedDomainStorage']) {
  if (!store.includes(token)) throw new Error(`Store contract missing: ${token}`);
}

console.log('Godmode command check OK', files.length);
